...
if(PRIOR_ONLY){
  ...
  BASE_DIR <- str_c("./fbm_logs/bin_class/posterior/")
}
...
# net-pred itmpn blog_1.net 51: blog_2.net 51: blog_3.net 51: blog_4.net 51: > posterior/combined_results.txt;

df_fbm <- data.frame(read.table(str_c(BASE_DIR, "combined_results.txt"), header = FALSE, blank.lines.skip = TRUE, skip = 5, nrows = 200)) %>% 
  janitor::clean_names() %>% 
  as_tibble()

names(df_fbm) <- c("case", "X_V1", "X_V2", "actual", "log_prob", "predicted", "wrong", "mean_prob", "error_2")
