...
// 3. Standard deviations are scaled using the number of weights in a given layer per Neal (1995), for finite expectations as number of units G -> infinite.
...
parameters {
  ...
  // matrix<lower=0>[max(max(G), K), max(max(G), K)] W_sdev[h+1];
  // matrix<lower=0>[max(G), h+1] B_sdev;
...
transformed parameters {
  ...
  // Precision transformation for weight parameters (useful weights only)
  ...
  // }
  // 
  // // Precision transformation for bias parameters (useful biases only)
  // 
  // for(l in 1:(h+1)){
  //   if (l == h+1){
...
model {
  ...
  for(l in 1:(h+1)){
    if (l == h+1){
      // For the output Layer -- only one bias term matters
        // B_prec[1, l] ~ gamma(B_gamma_shape, B_gamma_scale);
        B[1, l] ~ normal(0, 1);
    } else {
      // For all other layers, index only on useful weights
      for(g in 1:G[l]) {
        // B_prec[g, l] ~ gamma(B_gamma_shape, B_gamma_scale);
        B[g, l] ~ normal(0, 1);
      }
    }
  }
  ...
  // Managing excess weights in fixed-size matrices. Keep these hard-coded for now.
