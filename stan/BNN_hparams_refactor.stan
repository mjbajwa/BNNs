...
data {
  ...
  int G[h+2]; // Number of neurons per layer
...
parameters {
  ...
  // Weights (W)
  matrix[max(G), max(G)] W[h+1]; // Hidden layer weight matrices -- H+1 size because of additional layer for outputs 
  ...
  matrix<lower=1e-6>[max(G), max(G)] W_sdev[h+1];
